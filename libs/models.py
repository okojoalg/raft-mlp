...
from libs.const import PATCH_SIZE, DIM, DEPTH


class Block(nn.Module):
    def __init__(self, dim, norm_dim, expansion_factor=4, dropout=0.):
        super().__init__()
        if dim == norm_dim:
...
class Level(nn.Module):
    def __init__(self, in_channels, out_channels, depth=4, image_size=224, patch_size=4, expansion_factor=4,
        ...
        self.fn = nn.Sequential(*[
            ...
            *[nn.Sequential(*[
                # height mixer
                Rearrange('b (h w) c -> b (c w) h', h=h),
                Block(h, out_channels * w, expansion_factor, dropout),
                # width mixer
                Rearrange('b (c w) h -> b (c h) w', h=h, w=w),
                Block(w, out_channels * h, expansion_factor, dropout),
                # channel mixer
